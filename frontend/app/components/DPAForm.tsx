...
import { DPAData, PartyInfo } from "@/app/types/dpa";
import { supabase } from "@/app/lib/supabase";

interface DPAFormProps {
  data: DPAData;
  onChange: (data: DPAData) => void;
...
export default function DPAForm({ data, onChange, documentId, onSaved }: DPAFormProps) {
  const [saving, setSaving] = useState(false);
  const [saveMsg, setSaveMsg] = useState("");

  const update = (partial: Partial<DPAData>) => onChange({ ...data, ...partial });
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between">
        <SectionTitle>Design Partner Agreement</SectionTitle>
      ...
      <Textarea label="Purpose" value={data.purpose} onChange={(v) => update({ purpose: v })} placeholder="Design partner program scope" />
      ...
      <div className="border-t border-zinc-100 pt-5">
        <PartyFields label="Company" party={data.company} onChange={(p) => update({ company: p })} />
      </div>
      <div className="border-t border-zinc-100 pt-5">
        <PartyFields label="Design Partner" party={data.designPartner} onChange={(p) => update({ designPartner: p })} />
      </div>
    </div>
  );
}
