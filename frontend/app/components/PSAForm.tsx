...
import { PSAData, PartyInfo } from "@/app/types/psa";
import { supabase } from "@/app/lib/supabase";

interface PSAFormProps {
  data: PSAData;
  onChange: (data: PSAData) => void;
...
export default function PSAForm({ data, onChange, documentId, onSaved }: PSAFormProps) {
  const [saving, setSaving] = useState(false);
  const [saveMsg, setSaveMsg] = useState("");

  const update = (partial: Partial<PSAData>) => onChange({ ...data, ...partial });
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between">
        <SectionTitle>Professional Services Agreement</SectionTitle>
      ...
      <Textarea label="Scope of Work" value={data.scopeOfWork} onChange={(v) => update({ scopeOfWork: v })} placeholder="Services to be provided" />
      <Input label="Effective Date" value={data.effectiveDate} onChange={(v) => update({ effectiveDate: v })} type="date" />

      <div className="flex flex-col gap-1.5">
        <label className="text-sm font-medium text-zinc-700">Term</label>
      ...
      <Input label="Fees" value={data.fees} onChange={(v) => update({ fees: v })} placeholder="e.g. $10,000/month" />
      ...
      <div className="border-t border-zinc-100 pt-5">
        <PartyFields label="Client" party={data.client} onChange={(p) => update({ client: p })} />
      </div>
    </div>
  );
}
